...
build:
	mkdir -p bin
	go build -o bin/azl-otelcol $(MODFLAGS) -ldflags="$(LDFLAGS)" -trimpath -tags "$(BUILDTAGS)" ./cmd/azl-otel-collector

update-sources:
	@echo "Updating OTEL component versions in builder-config.yaml to $(OCB_VERSION)..."
	@sed -i 's/0\.[0-9]\+\.[0-9]\+/$(OCB_VERSION)/g' cmd/azl-otel-collector/builder-config.yaml
	...
	@chmod +x $(OCB_BIN)
	@echo "Running OCB..."
	@$(OCB_BIN) --config cmd/azl-otel-collector/builder-config.yaml --skip-compilation
	@rm -f cmd/azl-otel-collector/main_windows.go
